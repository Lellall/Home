...
const ModalVerified = ({ show, onClose }) => {
  ...
  const errorContent =
    'Failed to verify your account at this moment please try again';
  const successContent = (
    <>
      ...
      >
        Welcome to our platform. You can now access exclusive features and
        content.
      </p>
      <RoundedButton onClick={onClose} style={{ padding: '8px 10px' }}>
        Continue
      </RoundedButton>
    </>
  );
  ...
  useEffect(() => {
    if (show) {
      ...
      if (user && token) {
        ...
        axios
          .put(`${BaseUrl}/users/verify?user=${user}&token=${token}`, null, {
            params1,
          })
          .then((response) => {
